...
class Instaloader:
    ...
    def download_hashtag_top_posts(self, hashtag: Union[Hashtag, str],
                                   max_count: Optional[int] = None,
                                   post_filter: Optional[Callable[[Post], bool]] = None,
                                   fast_update: bool = False,
                                   profile_pic: bool = True,
                                   posts: bool = True) -> None:
        """
        Download top posts of a hashtag.

        To download the top posts for hashtag #cat, use:
            loader = Instaloader()
            loader.download_hashtag_top_posts('cat', max_count=30)
        ...
        # 다운로드 전에 프로필 사진 다운로드 (선택 사항)
        if profile_pic:
            with self.context.error_catcher("Download profile picture of {}".format(target)):
                self.download_hashtag_profilepic(hashtag)

        # 인기 게시물(top posts)을 다운로드
        if posts:
            self.context.log("Retrieving top posts with hashtag #{}...".format(hashtag.name))
            # get_top_posts()를 사용하여 인기 게시물 이터레이터를 얻음.
            posts_iterator = hashtag.get_relevance_posts()
            # 다운로드 루프 실행 (이미 구현된 posts_download_loop() 사용)
            self.posts_download_loop(posts_iterator, target, fast_update, post_filter, max_count=max_count)

        # 메타데이터 저장 (원본 JSON 덤프 등 포함 가능)
        if self.save_metadata:
